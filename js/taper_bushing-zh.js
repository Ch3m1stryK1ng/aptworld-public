...
const APT = (() => {
  const DATA_URL = '/assets/products_taper_bushing_zh.json';
    ...
    (async function init() {
      ...
      if (isFileProtocol) {
        ...
        try {
          ...
          const paths = [
            base + '../../assets/products_taper_bushing_zh.json',
            base + '../assets/products_taper_bushing_zh.json',
            '/assets/products_taper_bushing_zh.json',
            '../../assets/products_taper_bushing_zh.json',
            '../assets/products_taper_bushing_zh.json',
            './assets/products_taper_bushing_zh.json'
          ];
          ...
          console.error('✗ Failed to load products_taper_bushing_zh.json:', e);
  ...
  function bindEvents() {
    ...
    $('#apt-copy')?.addEventListener('click', () => {
      ...
      navigator.clipboard.writeText(text).then(() => alert('已复制到清单'));
    });
      ...
      <div class="product card">
        ...
        <div class="p-body">
          ...
          <div class="row" style="margin-top:6px">
            <span class="badge">尺寸: ${p.bush_size || ''}</span>
            <span class="badge">类型 ${p.type || ''}</span>
            <span class="badge">最大孔径: ${p.dbmax || ''} mm</span>
          </div>
          <div class="kv">
            <div class="kv-name">材质</div><div>${p.material || ''}</div>
            <div class="kv-name">表面处理</div><div>${p.surface_treatment || ''}</div>
            <div class="kv-name">最大孔径</div><div>${p.dbmax || ''} mm</div>
            <div class="kv-name">扭矩</div><div>${p.transmittable_torque || ''} Nm</div>
            <div class="kv-name">重量</div><div>${fmtKg(p.weight || 0)}</div>
          </div>
          <div class="row" style="margin-top:10px;align-items:center">
            ...
            <button class="btn sm" onclick="APT.openModal('${p.id}')">快速预览</button>
            <a class="btn sm" href="taper_bushing.html?sku=${encodeURIComponent(p.sku || '')}">详情</a>
            ...
            <button class="btn sm ${state.compare.has(p.id) ? 'primary' : ''}"
                ...
                ${state.compare.has(p.id) ? '已加入' : '加入对比'}
            </button>
          </div>
        </div>
      </div>`;
  ...
  function renderDetail(sku) {
    ...
    $('#apt-crumb').textContent = `产品中心 / 锥套 / ${p.series || 'TB'} / ${p.bush_size || ''}`;
    ...
    const kvEl = $('#apt-dKv'); if (kvEl) {
      kvEl.innerHTML = `
                <div class="kv-name">锥套尺寸</div><div>${p.bush_size || ''}</div>
                <div class="kv-name">类型</div><div>类型 ${p.type || ''}</div>
                <div class="kv-name">材质</div><div>${p.material || ''}</div>
                <div class="kv-name">表面处理</div><div>${p.surface_treatment || ''}</div>
                <div class="kv-name">最大孔径 (dbmax)</div><div>${p.dbmax || ''} mm</div>
                <div class="kv-name">可传输扭矩</div><div>${p.transmittable_torque || ''} Nm</div>
                <div class="kv-name">重量</div><div>${fmtKg(p.weight || 0)}</div>
            `;
    }
  ...
  function openModal(id) {
    ...
    $('#apt-mvProfile').textContent = `尺寸: ${p.bush_size || ''}`;
    $('#apt-mvGrooves').textContent = `类型 ${p.type || ''}`;
    $('#apt-mvDia').textContent = `最大孔径: ${p.dbmax || ''} mm`;
    $('#apt-mvBal').textContent = `扭矩: ${p.transmittable_torque || ''} Nm`;
    ...
    if (cmpBtn) {
      const setCmpBtn = (on) => {
        ...
        cmpBtn.textContent = on ? '已加入' : '加入对比';
  ...
  function renderCart() {
    ...
    if (!list) return;
    if (items.length === 0) {
      list.innerHTML = '<div class="muted" style="padding:12px">询价车为空</div>';
      const sumEl = $('#apt-cartSummary');
      if (sumEl) sumEl.innerHTML = '0 件商品';
      return;
    }
    list.innerHTML = items.map(it => `
            <div class="cart-item">
                ...
                <button class="btn sm danger" data-act="remove" data-id="${it.id}">移除</button>
                </div>
            </div>`).join('');
    const total = items.reduce((n, it) => n + (it.qty || 0), 0);
    const sumEl = $('#apt-cartSummary');
    if (sumEl) {
      sumEl.innerHTML = `共 <span class="emph-num">${items.length}</span> 款产品，数量 <span class="emph-num">${total}</span>`;
    }
  }
