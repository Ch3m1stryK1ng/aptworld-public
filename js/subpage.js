...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (sidebar) {
        sidebar.addEventListener('click', (e) => {
            ...
            // If it has a submenu, check if we should toggle or navigate
            if (sub) {
                const href = link.getAttribute('href');
                // Only prevent default and toggle if it's a placeholder link
                if (!href || href === '#' || href === 'javascript:void(0);') {
                    e.preventDefault();
                    li.classList.toggle('is-open');
                }
                // Otherwise let it navigate naturally
            }
        });
    }
